...
const AreaChart = ({ data, color, height = 150 }: { data: number[], color: string, height?: number }) => {
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full overflow-visible" preserveAspectRatio="none" aria-label="مخطط نمو المبيعات">
      <defs>
      ...
      {points.map((p, i) => (
        <circle key={i} cx={p.x} cy={p.y} r="3" fill={color} className="transition-all hover:r-5 cursor-pointer" aria-label={`نقطة بيانات ${data[i].toLocaleString()} في اليوم ${i + 1}`} />
      ))}
    </svg>
  );
};
...
const DonutChart = ({ data }: { data: { label: string, value: number, color: string }[] }) => {
  ...
  return (
    <svg viewBox="-1 -1 2 2" className="w-48 h-48 md:w-64 md:h-64 transform -rotate-90" aria-label="مخطط دائري لتوزيع القوة البيعية">
      {data.map((slice, i) => {
  ...
  return (
    <div className="space-y-6 md:space-y-12 animate-in fade-in duration-500 pb-32 px-2 md:px-0">
      ...
      <div className="bg-white dark:bg-slate-900 p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] shadow-xl border border-slate-100 dark:border-slate-800">
        <div className="flex flex-col xl:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-4 md:gap-6 w-full md:w-auto text-right">
            <div className="w-14 h-14 md:w-20 md:h-20 bg-indigo-600 text-white rounded-2xl md:rounded-[2rem] flex items-center justify-center text-3xl md:text-5xl shadow-2xl" aria-hidden="true">📈</div>
            <div>
          ...
          <div className="flex flex-col md:flex-row gap-2 items-center bg-slate-50 dark:bg-slate-800/50 p-3 md:p-4 rounded-2xl w-full xl:w-auto border dark:border-slate-700">
             <input type="date" value={dateRange.start} onChange={e => setDateRange({...dateRange, start: e.target.value})} className="bg-white dark:bg-slate-900 px-3 py-1.5 rounded-xl font-black text-xs md:text-lg dark:text-white outline-none border border-slate-200 dark:border-slate-700" aria-label="تاريخ البدء للتقرير" />
             <span className="text-slate-400 font-black" aria-hidden="true">➔</span>
             <input type="date" value={dateRange.end} onChange={e => setDateRange({...dateRange, end: e.target.value})} className="bg-white dark:bg-slate-900 px-3 py-1.5 rounded-xl font-black text-xs md:text-lg dark:text-white outline-none border border-slate-200 dark:border-slate-700" aria-label="تاريخ الانتهاء للتقرير" />
          </div>
        </div>
      </div>
      ...
      <nav className="flex bg-white dark:bg-slate-900 p-1.5 rounded-[1.5rem] md:rounded-[2.5rem] shadow-lg overflow-x-auto no-scrollbar gap-1 border border-slate-100 dark:border-slate-800" role="tablist">
        <ReportTabButton active={activeTab === 'overview'} onClick={() => setActiveTab('overview')} label="نظرة عامة" icon="🏠" id="overview-tab" />
        <ReportTabButton active={activeTab === 'analytics'} onClick={() => setActiveTab('analytics')} label="تحليل الأصناف" icon="📊" id="analytics-tab" />
        <ReportTabButton active={activeTab === 'predictions'} onClick={() => setActiveTab('predictions')} label="التوقعات" icon="🔮" id="predictions-tab" />
        <ReportTabButton active={activeTab === 'export'} onClick={() => setActiveTab('export')} label="تصدير" icon="📥" id="export-tab" />
      </nav>
      ...
      {activeTab === 'analytics' && (
        <div className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-xl border-2 border-slate-100 dark:border-slate-800 overflow-hidden" role="tabpanel" aria-labelledby="analytics-tab">
           ...
           <div className="hidden md:block overflow-x-auto p-4 md:p-8">
              <table className="excel-table w-full text-right" role="table" aria-label="تحليل الأصناف">
                 <thead>
                    <tr className="bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300">
                       <th scope="col" className="p-4 border-l dark:border-slate-700">نوع القات</th>
                       <th scope="col" className="p-4 border-l dark:border-slate-700 text-center">الكمية المباعة</th>
                       <th scope="col" className="p-4 border-l dark:border-slate-700 text-center">الإيرادات</th>
                       <th scope="col" className="p-4 border-l dark:border-slate-700 text-center">التكاليف</th>
                       <th scope="col" className="p-4 text-center font-black text-emerald-600">صافي الربح</th>
                    </tr>
                 </thead>
                 <tbody>
               ...
               <div key={type} className="p-4 space-y-2" role="listitem">
                 <div className="flex justify-between items-center mb-1">
                   ...
                   <span className="px-2 py-0.5 bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300 rounded text-[9px] font-black">
                     ربح: {data.profit.toLocaleString()} ر.ي
                   </span>
                 </div>
                 <div className="grid grid-cols-3 gap-2 items-center text-[10px] text-slate-500">
                   <span className="text-center">الكمية:<br/><span className="font-bold text-slate-800 dark:text-white">{data.qty.toLocaleString()}</span></span>
                   <span className="text-center">الإيرادات:<br/><span className="font-bold text-slate-800 dark:text-white">{data.revenue.toLocaleString()}</span></span>
                   <span className="text-center">التكاليف:<br/><span className="font-bold text-slate-800 dark:text-white">{data.cost.toLocaleString()}</span></span>
                 </div>
               </div>
             ))}
             {Object.entries(reportStats.qatPerformance).length === 0 && (
               <div className="p-10 text-center opacity-30 italic">لا توجد بيانات أصناف مطابقة.</div>
             )}
           </div>
        </div>
      )}
      ...
      {activeTab === 'predictions' && (
        <div className="bg-slate-900 p-8 md:p-16 rounded-[2.5rem] md:rounded-[4rem] text-white shadow-2xl relative overflow-hidden" role="tabpanel" aria-labelledby="predictions-tab">
           <div className="absolute top-0 right-0 p-10 opacity-5 pointer-events-none text-[8rem] md:text-[15rem] rotate-12" aria-hidden="true">🔮</div>
           <div className="relative z-10 space-y-6 md:space-y-10">
              <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                 ...
                 <button onClick={handleFetchForecast} disabled={isAnalyzing} className="bg-white text-indigo-900 px-8 py-4 rounded-2xl font-black text-sm md:text-xl shadow-xl active:scale-95 disabled:opacity-50 transition-all" aria-label="توليد التوقع الذكي">توليد التوقع الذكي ✨</button>
              </div>
              {forecast && (
      ...
      {activeTab === 'export' && (
        <div className="bg-white dark:bg-slate-900 p-10 md:p-20 rounded-[2.5rem] md:rounded-[4rem] shadow-xl text-center border-2 border-dashed border-slate-200 dark:border-slate-800" role="tabpanel" aria-labelledby="export-tab">
           <div className="text-6xl md:text-8xl mb-8" aria-hidden="true">📦</div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl mx-auto">
              <button className="bg-emerald-600 text-white p-6 md:p-10 rounded-3xl font-black text-lg md:text-2xl shadow-xl hover:scale-105 transition-all" aria-label="تصدير إلى إكسل">📊 إكسل (Excel)</button>
              <button className="bg-slate-900 text-white p-6 md:p-10 rounded-3xl font-black text-lg md:text-2xl shadow-xl hover:scale-105 transition-all" aria-label="تصدير إلى بي دي إف">📄 بي دي إف (PDF)</button>
           </div>
        </div>
      )}
    </div>
