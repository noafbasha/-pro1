...
const AiAssistant: React.FC = React.memo(() => {
  ...
  return (
    <div className="fixed inset-0 z-[250] flex items-end md:items-center justify-center p-0 md:p-4 animate-in fade-in duration-300">
      <div className="absolute inset-0 bg-slate-950/95 backdrop-blur-xl" onClick={() => setIsAiOpen(false)} aria-label="إغلاق المساعد الذكي"></div>
      <div className="relative w-full md:max-w-4xl h-full md:h-[85vh] bg-white dark:bg-slate-900 rounded-t-[2.5rem] md:rounded-[3rem] shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="p-6 md:p-8 bg-slate-900 text-white flex justify-between items-center">
           ...
           <button onClick={() => setIsAiOpen(false)} className="w-10 h-10 md:w-12 md:h-12 bg-white/10 rounded-full flex items-center justify-center text-2xl hover:bg-white/20 transition-all" aria-label="إغلاق">✕</button>
        </div>
        ...
        <div className="flex-grow overflow-y-auto p-4 md:p-8 space-y-6 no-scrollbar bg-slate-50 dark:bg-slate-950/20" role="log" aria-live="polite">
          ...
          {isLoading && <div className="text-brandGreen animate-pulse font-black px-4 text-xs md:text-lg" aria-label="المستشار يحلل البيانات">🧠 المستشار يحلل البيانات...</div>}
        ...
        <div className="p-4 md:p-8 border-t dark:border-slate-800 bg-white dark:bg-slate-900 pb-10 md:pb-8">
          <div className="flex gap-3 md:gap-6 items-center">
             <button 
                ...
                aria-label={isLiveActive ? 'إيقاف التسجيل الصوتي' : 'بدء التسجيل الصوتي'}
             >
                {isLiveActive ? '⏹️' : '🎙️'}
             </button>
             <div className="flex-grow relative">
                <input 
                  ...
                  aria-label="حقل إدخال الأوامر للمساعد الذكي"
                />
                <button onClick={handleSend} className="absolute left-3 top-1/2 -translate-y-1/2 bg-slate-950 dark:bg-emerald-600 text-white p-3 md:p-4 rounded-xl md:rounded-2xl font-black text-sm md:text-lg shadow-lg" aria-label="إرسال الأمر">إرسال</button>
             </div>
          </div>
        </div>
          ...
          <div className="absolute inset-0 bg-slate-950/95 backdrop-blur-xl flex items-center justify-center p-6 z-[300]" role="dialog" aria-modal="true" aria-labelledby="confirmation-title">
            <div className="bg-white dark:bg-slate-900 w-full sm:max-w-md rounded-[2.5rem] p-8 md:p-10 shadow-2xl border-t-[10px] border-brandGreen animate-in zoom-in">
               ...
               <div className="grid grid-cols-2 gap-4">
                  <button onClick={executeAction} className="bg-brandGreen text-white py-5 rounded-2xl font-black text-lg md:text-2xl shadow-xl active:scale-95" aria-label="تأكيد تنفيذ العملية">تأكيد ✅</button>
                  <button onClick={() => setPendingAction(null)} className="bg-slate-100 dark:bg-slate-800 text-slate-500 py-5 rounded-2xl font-black text-lg" aria-label="تجاهل العملية">تجاهل</button>
               </div>
            </div>
          </div>
        )}
