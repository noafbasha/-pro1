    ...
    <div className="space-y-10 animate-in fade-in duration-500 pb-24">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-800 gap-6 transition-all print:hidden">
        ...
        <div className="flex flex-wrap gap-3 w-full lg:w-auto">
          <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-2xl border dark:border-slate-700" role="group" aria-label="تبديل عرض الديون">
             <button 
          ...
          <div className="relative flex-grow">
            <input 
              ...
              aria-label="حقل البحث عن عميل"
            />
            <span className="absolute right-4 top-1/2 -translate-y-1/2 opacity-30" aria-hidden="true">🔍</span>
          </div>

          <button onClick={() => setIsVoucherOpen(true)} className="bg-amber-600 text-white px-8 py-4 rounded-2xl font-black shadow-lg hover:bg-amber-700 transition flex items-center gap-2 whitespace-nowrap" aria-label="تسجيل قبض مالي جديد">
            <span>➕</span> قبض مالي
          </button>
        </div>
      </div>
      ...
      {viewMode === 'table' ? (
        <div className="bg-white dark:bg-slate-900 rounded-[3rem] shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden animate-in slide-in-from-bottom duration-500">
           <div className="overflow-x-auto p-4 hidden md:block">
              <table className="excel-table w-full border-2 border-slate-300 dark:border-slate-700" role="table" aria-label="جدول ديون العملاء">
                <thead>
                  <tr className="bg-slate-100 dark:bg-slate-800">
                    <th scope="col" className="border border-slate-300 dark:border-slate-700 p-4 text-right">اسم العميل</th>
                    <th scope="col" className="border border-slate-300 dark:border-slate-700 p-4 text-center bg-slate-50/50">يمني (YER)</th>
                    <th scope="col" className="border border-slate-300 dark:border-slate-700 p-4 text-center bg-blue-50/30">سعودي (SAR)</th>
                    <th scope="col" className="border border-slate-300 dark:border-slate-700 p-4 text-center bg-indigo-50/30">عماني (OMR)</th>
                    <th scope="col" className="border border-slate-300 dark:border-slate-700 p-4 text-center bg-amber-50 dark:bg-amber-900/10 font-black text-amber-700">الإجمالي (باليمني)</th>
                    <th scope="col" className="border border-slate-300 dark:border-slate-700 p-4 text-center">الحالة</th>
                    <th scope="col" className="border border-slate-300 dark:border-slate-700 p-4 text-center print:hidden">إجراءات</th>
                  </tr>
                </thead>
                <tbody>
                        ...
                        <td className="border border-slate-200 dark:border-slate-700 p-4 text-center print:hidden">
                           <div className="flex justify-center gap-2">
                              <button 
                                ...
                                title="مطالبة صوتية"
                                aria-label={`توليد مطالبة صوتية للعميل ${debt.customerName}`}
                              >🎙️</button>
                              <button 
                                ...
                                title="تحصيل مبلغ"
                                aria-label={`تحصيل مبلغ من العميل ${debt.customerName}`}
                              >📥</button>
                           </div>
                        </td>
                      </tr>
           ...
           <div className="md:hidden divide-y dark:divide-slate-800" role="list">
                   ...
                   <div className="flex justify-between items-start mb-2">
                     <div className="flex items-center gap-2">
                       ...
                       <div>
                         ...
                         <div className="text-[9px] text-slate-400 font-bold">آخر نشاط: {new Date(debt.lastActivityDate).toLocaleDateString('ar-YE')}</div>
                       </div>
                     </div>
                     <span className={`px-2 py-1 rounded-full font-black text-[9px] ${aging.color}`}>{aging.label} ({aging.days} يوم)</span>
                   </div>
                   <div className="flex justify-between items-center mt-3">
                     <div className="font-black text-lg text-amber-700">
                       {debt.totalYer.toLocaleString()} <span className="text-[10px]">ر.ي</span>
                     </div>
                     <div className="flex gap-2">
                       <button 
                         onClick={() => handleGenerateVoice(debt)}
                         className="p-2 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 rounded-lg"
                         aria-label={`مطالبة صوتية للعميل ${debt.customerName}`}
                       >🎙️</button>
                       <button 
                         onClick={() => { setVoucherData({ ...voucherData, entityId: debt.customerId }); setIsVoucherOpen(true); }}
             ...
             {filteredDebts.length === 0 && (
               <div className="p-10 text-center opacity-30 italic">لا توجد ديون مطابقة.</div>
             )}
           </div>
        </div>
      ) : (
              ...
              <div key={debt.customerId} className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] shadow-xl border-2 border-slate-100 dark:border-slate-800 flex flex-col justify-between group hover:border-amber-500 transition-all relative overflow-hidden" role="group" aria-labelledby={`debt-card-title-${debt.customerId}`}>
                 ...
                 <div className="relative z-10 space-y-4">
                    <div className="flex justify-between items-start">
                      <div className="w-16 h-16 bg-slate-100 dark:bg-slate-800 rounded-2xl flex items-center justify-center text-3xl shadow-inner group-hover:rotate-6 transition-transform" aria-hidden="true">👤</div>
                 ...
                 <div className="mt-8 flex gap-3 relative z-10">
                    ...
                    <button 
                                onClick={() => { setVoucherData({ ...voucherData, entityId: debt.customerId }); setIsVoucherOpen(true); }}
                                className="flex-grow bg-amber-600 text-white py-4 rounded-2xl font-black text-sm shadow-xl hover:bg-amber-700 transition active:scale-95" aria-label={`تحصيل مبلغ من العميل ${debt.customerName}`}>📥 تحصيل</button>
                 </div>
              </div>
            );
          })}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[150] flex items-center justify-center p-4 animate-in fade-in overflow-y-auto" role="dialog" aria-modal="true" aria-labelledby="voucher-modal-title">
          <div className="bg-white dark:bg-slate-900 w-full sm:max-w-lg rounded-[2.5rem] md:rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in border-4 border-amber-500/20 my-auto">
            <div className="p-6 md:p-10 bg-amber-600 text-white flex justify-between items-center">
              <h3 id="voucher-modal-title" className="text-2xl md:text-3xl font-black flex items-center gap-4"><span>📥</span> استلام دفعة مالية</h3>
              <button onClick={() => setIsVoucherOpen(false)} className="text-3xl md:text-4xl hover:rotate-90 transition-transform" aria-label="إغلاق">✕</button>
            </div>
            <form onSubmit={handleVoucherSubmit} className="p-6 md:p-10 space-y-6 md:space-y-8">
              ...
              <div className="space-y-4">
                <label htmlFor="amount-paid" className="text-xs font-black text-slate-500 uppercase tracking-widest px-4 text-center block">المبلغ المدفوع</label>
              ...
              <div className="space-y-4">
                 <label htmlFor="voucher-notes" className="text-xs font-black text-slate-500 uppercase tracking-widest px-4">ملاحظة (بيان السند)</label>
                 <input id="voucher-notes" className="w-full p-5 bg-slate-50 dark:bg-slate-800 border-2 rounded-2xl outline-none font-bold dark:text-white focus:border-amber-500 shadow-inner text-base" value={voucherData.notes} onChange={e => setVoucherData({...voucherData, notes: e.target.value})} placeholder="مثال: تسديد دفعة أمس.." />
              </div>

              <button type="submit" className="w-full bg-amber-600 text-white py-6 md:py-7 rounded-[2rem] font-black text-2xl md:text-3xl shadow-2xl hover:bg-amber-700 transition active:scale-95 shadow-amber-600/30" aria-label="اعتماد سند القبض">
                ✅ اعتماد سند القبض
              </button>
            </form>
          </div>
