...
const ExchangePage: React.FC = React.memo(() => {
  ...
  const getPriceTrend = (index: number, type: 'SAR' | 'OMR') => {
    ...
    if (current > previous) return <span className="text-green-500 text-[8px] md:text-xs" aria-label="ارتفاع في السعر">▲</span>;
    if (current < previous) return <span className="text-red-500 text-[8px] md:text-xs" aria-label="انخفاض في السعر">▼</span>;
    return <span className="text-slate-300 text-[8px] md:text-xs" aria-label="ثبات في السعر">▬</span>;
  };
    ...
    <div className="space-y-6 md:space-y-12 animate-in slide-in-from-top duration-500 max-w-7xl mx-auto pb-32 px-2 md:px-0">
      <div className="text-center space-y-2 md:space-y-4 bg-white dark:bg-slate-900 p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] shadow-sm border border-slate-100 dark:border-slate-800 transition-colors">
        <h2 className="text-2xl md:text-5xl font-black text-slate-800 dark:text-slate-100 flex items-center justify-center gap-4 transition-colors">
            <span className="text-3xl md:text-6xl" aria-hidden="true">💱</span> إدارة أسعار الصرف
        </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-10">
        <div className="space-y-6 md:space-y-10">
              ...
              <form onSubmit={handleUpdate} className="space-y-4 md:space-y-8">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label htmlFor="sar-rate" className="block text-[8px] md:text-sm font-black text-slate-500 uppercase font-bold">السعودي</label>
                    <input 
                      id="sar-rate"
                      type="number" 
                      ...
                      aria-label="سعر صرف الريال السعودي مقابل اليمني"
                      aria-required="true"
                    />
                  </div>
                  <div className="space-y-1">
                    <label htmlFor="omr-rate" className="block text-[8px] md:text-sm font-black text-slate-500 uppercase font-bold">العماني</label>
                    <input 
                      id="omr-rate"
                      type="number" 
                      className="w-full p-4 md:p-6 border-2 md:border-4 border-slate-50 dark:border-slate-800 rounded-xl md:rounded-2xl outline-none focus:border-blue-500 bg-slate-50 dark:bg-slate-800 dark:text-white transition-all text-xl md:text-3xl font-black"
                      ...
                      aria-label="سعر صرف الريال العماني مقابل اليمني"
                      aria-required="true"
                    />
                  </div>
                </div>
                <button type="submit" className="w-full bg-indigo-700 text-white py-4 md:py-6 rounded-xl md:rounded-2xl font-black text-sm md:text-xl hover:bg-indigo-800 transition active:scale-95" aria-label="اعتماد الأسعار الجديدة">💾 اعتماد الأسعار</button>
              </form>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[2rem] md:rounded-[3rem] shadow-2xl p-6 md:p-10 border border-slate-100 dark:border-slate-800 flex flex-col transition-colors">
            ...
            <div className="space-y-4 md:space-y-8">
                <div className="space-y-1">
                    <label htmlFor="yer-amount" className="block text-[10px] md:text-lg font-black text-slate-700 dark:text-slate-300 font-bold">المبلغ باليمني (YER)</label>
                    <input 
                        id="yer-amount"
                        type="number"
                        ...
                        aria-label="المبلغ بالريال اليمني للتحويل"
                    />
                </div>
                <div className="grid grid-cols-2 gap-3 md:gap-6">
                    <div className="bg-amber-50 dark:bg-amber-950/20 p-4 md:p-8 rounded-xl md:rounded-[2rem] border-2 border-amber-100 dark:border-amber-900/30 text-center" aria-label={`القيمة المحولة للريال السعودي: ${calc.resultSAR.toFixed(2)}`}>
                    ...
                    <div className="bg-blue-50 dark:bg-blue-950/20 p-4 md:p-8 rounded-xl md:rounded-[2rem] border-2 border-blue-100 dark:border-blue-900/30 text-center" aria-label={`القيمة المحولة للريال العماني: ${calc.resultOMR.toFixed(2)}`}>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[2rem] md:rounded-[3rem] shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col">
           ...
           <div className="flex-grow overflow-y-auto max-h-[400px] md:max-h-[1000px] no-scrollbar">
              <table className="excel-table w-full" role="table" aria-label="سجل تاريخ أسعار الصرف">
                 <thead className="sticky top-0 z-10">
                    <tr>
                       <th scope="col" className="text-right p-4">التاريخ</th>
                       <th scope="col" className="text-center p-4">سعودي</th>
                       <th scope="col" className="text-center p-4">عماني</th>
                    </tr>
                 </thead>
                 <tbody>
                    {rateHistory.map((h, i) => (
